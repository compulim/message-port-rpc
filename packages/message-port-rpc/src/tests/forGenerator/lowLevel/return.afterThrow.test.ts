...
scenario(
  ...
  bdd => {
    bdd
      ...
      .when('generator.return(345) is called', async ({ generator }) => {
        return await generator.return(345);
      })
      .then('should return 345', (_, value) => {
        expect(value).toEqual({ done: true, value: 345 });
      })
      .when('generator.next() is called', async ({ generator }) => {
        return await generator.next();
      })
