...
describe('when iterating', () => {
  ...
  describe('next() for the first time', () => {
    ...
    describe('throw()', () => {
      beforeEach(async () => {
        result = await generator.throw('Aloha!');
      });

      test('should reach catch block with erorr', () => {
        expect(catchFn.mock.callCount()).toBe(1);
        expect(catchFn.mock.calls[0]?.arguments).toEqual(['Aloha!']);
      });
      test('should reach finally block', () => expect(finallyFn.mock.callCount()).toBe(1));
      test('should return 2', () => expect(result).toEqual({ done: true, value: 2 }));
    });
  });
});
